(define (accumulate operation identity-element)
  (define (op term i next n)
    (if (> i n)
        identity-element
        (operation (term i)
                   (op term (next i) next n))))
  (lambda (term i next n) (op term i next n))
)
(define pow (accumulate * 1))
